...
Dir_results = 'G:\\Shared drives\\Head Models (KTH, GHBMC, ML)\\ML\\Results'
Dir_data_X = 'G:\\Shared drives\\Head Models (KTH, GHBMC, ML)\\ML\\Training Dataset\\MLHM2 X'
Dir_data_Y_MPSR = 'G:\\Shared drives\\Head Models (KTH, GHBMC, ML)\\ML\\Training Dataset\\MLHM2 Y\\MPSR'
Dir_code = 'G:\\Shared drives\\Head Models (KTH, GHBMC, ML)\\ML\\Code'
np.random.seed(7)
...
task = 'MLHM2'
method = 'hand510'
dataset = 'HM2MMA' #HM/HM2MMA/HM2MMA/HM2NASCARR
outcome = 'MPSR'
...
MMA_X = io.loadmat('Enora_Boxing_X.mat')['MLHM2_X']
assert MMA_X.shape[0] == 260
...
MMA_Y = io.loadmat('Enora_Boxing_Y.mat')['label']
assert MMA_X.shape[0] == 260

###--- DRCA Transfer to MMA dataset ---###
...
for D in components:
    for alpha in alphas:
        ...
        Adam = optimizers.Adam(lr = lr, decay=5e-8)
        model.fit(X_std_aug, Y_std_aug, epochs=epoch, batch_size=512, verbose=False)
        ...
        savemat('Boxing_MPSR_prediction.mat', {'Y_test': Y_test, 'Y_pred': Y_predict_test_base})

# print('\t'.join(['MAB']+[str(round(MAE_recorder[i],4)) for i in range(4)]))
# print('\t'.join(['RMSB']+[str(round(RMSE_recorder[i],4)) for i in range(4)]))
# print('\t'.join(['R2B']+[str(round(R2_recorder[i],4)) for i in range(4)]))