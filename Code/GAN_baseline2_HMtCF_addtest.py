...
for D in components:
    for alpha in alphas:
        ...
        savemat('PAC12_MPS_prediction.mat', {'Y_test': Y_test, 'Y_pred': Y_predict_test_base})

# print('\t'.join(['MAB']+[str(round(MAE_recorder[i],4)) for i in range(6)]))
# print('\t'.join(['RMSB']+[str(round(RMSE_recorder[i],4)) for i in range(6)]))
# print('\t'.join(['R2B']+[str(round(R2_recorder[i],4)) for i in range(6)]))